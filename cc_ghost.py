...
Usage:
  ...
  cc-ghost --platform twitter           # optimize for Twitter
...
CONFIG_DIR = Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "cc-ghost"
CLAUDE_PROJECTS_DIR = Path.home() / ".claude" / "projects"
CONFIG_ENV_PATH = CONFIG_DIR / "config.env"
PERSONA_PATH = CONFIG_DIR / "persona.md"
POSTS_DIR = CONFIG_DIR / "posts"
LAST_RUN_PATH = CONFIG_DIR / ".last_run"

# Load config.env from config dir, fall back to .env in cwd for dev
...
def setup_api_key():
    ...
    if CONFIG_ENV_PATH.exists():
        for line in CONFIG_ENV_PATH.read_text(encoding="utf-8").split("\n"):
            if not line.startswith("ANTHROPIC_API_KEY="):
                env_lines.append(line)

    env_lines.insert(0, f"ANTHROPIC_API_KEY={key}")
